...
static void glitch_log_impl(const char *fmt, va_list ap)
{
	size_t txt_sz;
	int ret;
	char stack_buf[8192], *malloc_buf = NULL, *buf;
	va_list ap2;

	va_copy(ap2, ap);
	txt_sz = vsnprintf(NULL, 0, fmt, ap);
	if (txt_sz < sizeof(stack_buf))
		buf = stack_buf;
	else {
		malloc_buf = malloc(txt_sz + 1);
		if (!malloc_buf) {
			static const char OOM_MSG[] = "error writing to log: "
							"out of memory\n";
			glitch_log_to_syslog_and_stderr(OOM_MSG,
				sizeof(OOM_MSG) - 1);
			return;
		}
		buf = malloc_buf;
